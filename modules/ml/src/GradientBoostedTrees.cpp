...
QoreListNode* QoreGradientBoostedTrees::getTreesData(ExceptionSink* xsink) const {
    std::lock_guard<std::mutex> lk(mtx);
    if (!fitted) {
        xsink->raiseException("ML-GBT-ERROR",
...
QoreListNode* QoreGradientBoostedTrees::getInitialPredictions(ExceptionSink* xsink) const {
    std::lock_guard<std::mutex> lk(mtx);
    if (!fitted) {
        xsink->raiseException("ML-GBT-ERROR",
            "model has not been fitted: call fit() or fitMatrix() first");
    ...
    ReferenceHolder<QoreListNode> rv(new QoreListNode(autoTypeInfo), xsink);
    for (double p : initial_predictions) {
        rv->push(p, xsink);
    }
    return rv.release();
}

QoreListNode* QoreGradientBoostedTrees::getClasses(ExceptionSink* xsink) const {
    std::lock_guard<std::mutex> lk(mtx);
    if (!fitted) {
        xsink->raiseException("ML-GBT-ERROR",
            "model has not been fitted: call fit() or fitMatrix() first");
