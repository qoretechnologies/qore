...
namespace draft4 {
    ...
    public:
        schema_validator_factory_4(std::unique_ptr<Json>&& root_schema,
            ...
            : schema_validator_factory_base<Json>(schema_version::draft4(), std::move(root_schema), factory_factory, options, schema_store_ptr, resolve_funcs),
              factory_(this)
        {
            init();
        }
    ...
    private:
            ...
            const Json& sch, jsoncons::span<const jsoncons::string_view> keys) const final
        {
            ...
            if (sch.is_object())
            {
                schema_readers_type::read_id_4(parent, sch, id, new_uris);
