...
namespace draft7 {
    ...
    public:
        schema_validator_factory_7(std::unique_ptr<Json>&& root_schema,
            ...
            : schema_validator_factory_base<Json>(schema_version::draft7(), 
                  std::move(root_schema), factory_factory, options, schema_store_ptr, resolve_funcs),
              factory_(this)
        {
            init();
        }
            ...
            const Json& sch, anchor_uri_map_type& anchor_dict)
        {
            ...
            {
                auto factory_it = keyword_factory_map_.find(std::string(key_value.key()));
