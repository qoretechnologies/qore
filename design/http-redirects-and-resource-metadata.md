...
| UNIX domain sockets | a request can only be redirected to a UNIX domain socket (`http://socket=...`) if it is on the same socket; a server can never send a request, or a repeated body, to a local socket |
