...
- [ ] `auto_url: True` only if URL can be built from connection options (domain/region/host) or the API has a single fixed endpoint
- [ ] **Self-hosted servers (Ollama, SGLang, vector DBs, sidecars) and per-environment services use `auto_url: False`.** With `auto_url` the platform hides the URL field and ignores any URL the user supplies, so a `localhost` default URL can never be changed.
- [ ] **Self-hosted connections have one scheme per transport** (ex: `chroma` → `http`, `chromas` → `https`, each registered and in `SchemeMap`). Mapping `http` and `https` to the same scheme makes the reverse lookup in `getRealUrl()` ambiguous and plain-HTTP servers are contacted with TLS.
- [ ] **For fixed-endpoint APIs (single global URL, no per-tenant domain), the connection class declares `DefaultConnectionUrl` and `getConfig()` hard-codes the URL.** The platform reads `DefaultConnectionUrl` (then `DefaultUrl`) from the connection class by reflection (parent classes included, but not the unrelated client base class); without either it uses `<scheme>://x`. Pair with `setUpdateOptionsCode()` to auto-correct any options update. `examples/test/qlib/ConnectionProvider/AutoUrlConnections.qtest` checks every `auto_url` connection.
