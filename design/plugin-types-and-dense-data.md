...
| `module-pgsql` | yes | yes | yes for boolean, integer, floating-point, string-like, safe `NUMERIC(p,s)`, date/time, and interval columns | Native PostgreSQL columnar fetch builds packed bool, int16/int32/int64, float32/float64, string, decimal128, and microsecond temporal buffers where metadata is safe and is the primary benchmarked native path. |
| `module-mysql` | yes | yes | yes for integer, floating-point, safe `DECIMAL(p,s)` / `NUMERIC(p,s)`, and date/time columns | Native MySQL columnar fetch builds packed int64, float64, and decimal128 buffers where safe; list-backed date/time columns with safe metadata are normalized to microsecond temporal buffers by libqore; binary, text shapes not exposed as typed strings, and oversized numeric columns use the standard columnar conversion path. |
| `module-oracle` | yes | yes | yes for integer, floating-point, safe `NUMBER(precision,scale)`, and date/time columns | Native OCI columnar fetch builds packed buffers where safe, including decimal128 for fitting `NUMBER` metadata; list-backed date/time columns with safe metadata are normalized to microsecond temporal buffers by libqore; unconstrained numeric, string, binary, and object columns use the standard columnar conversion path. |
| `module-odbc` | yes | yes | yes for integer, floating-point, safe `SQL_NUMERIC` / `SQL_DECIMAL`, and date/time columns | Native ODBC columnar fetch builds packed int64, float64, and decimal128 buffers where driver metadata is safe; list-backed date/time columns with safe metadata are normalized to microsecond temporal buffers by libqore; text, binary, and unsupported driver-specific columns use the standard columnar conversion path. |
...
| `module-sybase` / freetds | yes | yes | yes for integer, floating-point, safe `DECIMAL(p,s)` / `NUMERIC(p,s)`, and date/time columns | Native Sybase/FreeTDS columnar fetch builds packed int64, float64, and decimal128 buffers where safe; list-backed date/time columns with safe metadata are normalized to microsecond temporal buffers by libqore; string, binary, and unsupported numeric shapes use the standard columnar conversion path. |
| `module-jni` / JDBC | yes | yes | yes for JDBC fixed-width numeric, boolean, string, decimal128, and date/timestamp columns | JDBC date/timestamp columns keep logical schema metadata and use microsecond temporal buffers when linked with Qore dense temporal support. Builds remain compatible with qore develop through CMake feature probes. |
